...
test("the release workflow performs npm signature verification before SDK provenance binding", () => {
  ...
  assert.match(workflow, /SDK_REF: v0\.5\.31/u);
...
test("the release fixtures pin CLI 0.4.10 while retaining SDK 0.5.31", () => {
  ...
  assert.equal(packageJson.dependencies?.["@telecrypt-io/storage"], "0.5.31");
  assert.equal(packageLock.packages?.["node_modules/@telecrypt-io/storage"]?.version, "0.5.31");
  assert.match(workflow, /SDK_REF: v0\.5\.31/u);
  assert.match(workflow, /"@telecrypt-io\/storage": "0\.5\.31"/u);
});
